...
import optuna
from scipy.stats import rankdata                    # FIX: was missing — crashed pct_rank()
...
# Consequence of median fill — side-by-side comparison
fig, axes = plt.subplots(1, 2, figsize=(13, 5.5))

axes[0].hist(df_chg["charge_std"], bins=50, color="#2ca02c", alpha=0.8,
             edgecolor="white")
...
fig.suptitle(
    ...
    fontsize=12, fontweight="bold")
fig.tight_layout(rect=[0,0,1,0.93])
# FIX: renamed to match manuscript figure reference (was fig_hoa_charge_imputation_effect)
save(fig, "fig_charge_imputation")
...
fig.suptitle(f"XGBoost Predictions vs. GCMC (n={len(idx_te):,} test structures)\n"
             "Selectivity shown in raw space (back-transformed from log₁₊ₓ training)",
             fontsize=12, fontweight="bold")
...
for i, tgt in enumerate(TARGETS):
    ...
    ax.hist(resid_clip, bins=80, color=COLORS[i], alpha=0.8,
...
for tgt in TARGETS:
    ...
    for name, model in baselines.items():
        ...
        if ns:
            sc = StandardScaler()
            Xt = sc.fit_transform(X_tr2); Xv = sc.transform(X_te2)
        else:
            Xt, Xv = X_tr2, X_te2
        if tgt == "selectivity_co2h2":
...
for ax, yl, tl in [(ax1,r"$R^2$",r"Test $R^2$"),
                   (ax2,"MAE","Test MAE")]:
...
print(f"  Pareto front: {len(front)} MOFs")
...
wc_thresh  = np.percentile(wc_p, 75)
sel_thresh = np.percentile(sel_p, 75)
...
print(f"  Sel threshold  : {sel_thresh:.1f} (real units)")
...
# ═════════════════════════════════════════════════════════════════════════════
# ADDITIONAL FIX: fig_selectivity_by_topology with 95% bootstrap CIs
# (manuscript Fix 3 — Figure 20 caption updated to match)
# ═════════════════════════════════════════════════════════════════════════════
print("\n" + "="*60)
print("ADDITIONAL FIX: fig_selectivity_by_topology (95% bootstrap CIs)")
print("="*60)
...
print(f"  #1 selectivity: {top50['pred_selectivity_co2h2'].iloc[0]:.1f} (real space)")


def pct_rank(arr, higher_is_better=True):
    # FIX: use rankdata from scipy.stats (now properly imported at top)
...
pct = {
    ...
    "heat_of_ads"      : pct_rank(hoa_p, False),
}
...
for tgt in TARGETS:
    ...
    note = " (log-space model, raw-space eval)" \
           if tgt == "selectivity_co2h2" else ""
...
fixed = [
    ...
    "fig_hoa_vs_charges",
    "fig_charge_imputation",
    "fig_selectivity_by_topology",   # now with 95% bootstrap CIs
]
for f in fixed:
